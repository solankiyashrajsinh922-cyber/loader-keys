...
function makeKey(type = 'normal'){ 
  if(type === 'mod_menu') {
    return 'AURA_' + rp() + '_' + rp();  // ✅ Changed prefix from H4X_MODZ_ to AURA_
  }
  if(type === 'vortex') {
    return 'vortexexclusive_' + rp() + '_' + rp();
  }
  return 'H4X_MODZ_NORMAL_' + rp(); 
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if(action==='validate'){
      ...
      if(!key.startsWith('H4X_MODZ_NORMAL_') && !key.startsWith('H4X_MODZ_') && !key.startsWith('vortexexclusive_') && !key.startsWith('AURA_')){
      ...
      // ✅ Sirf 2 mods — key_type ke hisaab se
      let allowedMods = {};
      let allowedFeatures = {};

      if (keyType === 'mod_menu' || keyType === 'vortex') {
        // 20 char key → mod_menu zip
        ...
        // 21 char key → room zip
        if (v.mods && v.mods.room) allowedMods.room = v.mods.room;
        if (v.mod_features && v.mod_features.room) allowedFeatures.room = v.mod_features.room;
      }

      // ✅ Real password seedha yahan — version.json wala fake hai, ye use hoga
      const zipPassword = "8190iwiej82uduud";

      // ✅ Auth file — key_type ke hisaab se
      const authFile = (keyType === 'mod_menu' || keyType === 'vortex')
        ? "woman_tianlin_@tianlin_swittchbs4.ab"
        : "woman_tianlin_@tianlin_swittchbs5.ab";
