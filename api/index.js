...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ✅ FIXED: Dono key formats allow karo
      if(!key.startsWith('Yash_X_Prime_') && !key.startsWith('H4X_MODZ_')){
    ...
    if(action==='reseller_generate_key'){
      ...
      if(reqKeyType === 'mod_menu' && !resellers[slug].allow_mod_menu){
        res.json({success:false,message:"Mod Menu key generation not allowed for this reseller."});
        return;
      }
