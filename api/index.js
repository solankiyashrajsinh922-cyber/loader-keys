...
async function getResellers() {
  try {
    ...
    return await getFile('resellers.json');  // ✅ FIXED: re-fetch for sha
  }
}
  ...
  try {
    ...
    if(action==='validate'){
      ...
      if (keyType === 'mod_menu' || keyType === 'vortex') {
        // Mod Menu & Vortex: only original and mod_menu
        ...
        allowedLocked = []; // no locks for mod_menu & vortex
      } else {
